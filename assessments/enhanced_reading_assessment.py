...
def get_reading_level():
    """
    Conducts an adaptive reading test using speech recognition and GUI dialogs.
    Returns a reading level from 1 (beginner) to 3 (advanced).
    """
    ...
    while current_level < len(levels):
        ...
        # After 5 words, decide whether to advance to a higher level
        if total == 5:
            if correct >= 3:
                current_level += 1
    ...
    root.destroy()
    return current_level + 1  # Return level: 1 (beginner) to 3 (advanced)

def test_comprehension():
    """
    Conducts a simple reading comprehension test using GUI dialogs.
    """
    root = tk.Tk()
    root.withdraw()
...
if __name__ == "__main__":
    # Run the adaptive reading test and comprehension test
